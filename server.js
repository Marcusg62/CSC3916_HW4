...
router.get('/movies/:movieTitle', authJwtController.isAuthenticated, function (req, res) {
    ...
    if (!req.params.movieTitle) {
        ...
        if (req.query.reviews == 'true') {
            ...
            Movie.aggregate().match({ title: req.params.movieTitle })
...
router.route('/reviews').get((req, res) => {
    res.send('getting movie reviews')

}).post(authJwtController.isAuthenticated, (req, res) => {
    if (!req.body.title || !req.body.name || !req.body.quote || !req.body.rating) {
        return res.json({ success: false, message: "Error: You must include a movie title, a reviewer name, quote and rating. " });
    }
            ...
            if (!movie) {
                ...
                review.save(function (err) {
                    if (err) {
                        ...
                        return res.status(200).send({ success: true, message: "Successfully created review." });
                    }
                });
            }
...
router.route('/reviews/:title').get((req, res) => {
    ...
    Review.find({ "title": req.params.title }).select("name quote rating").exec(function (err, review) {
        if (err) {
            return res.status(403).json({ success: false, message: "Unable to retrieve title passed in." });
        }
